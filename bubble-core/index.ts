...
function serializeBubblePropertyValue(value: unknown): BubbleSerializedProperty {
  if (value === null) {
    return null;
  }
  ...
  if (typeof value === "object") {
    const serializedEntries = Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, nestedValue]) => [key, serializeBubblePropertyValue(nestedValue)] as const);

    return Object.freeze(Object.fromEntries(serializedEntries));
  }

  throw new Error(`Unsupported property value in snapshot serialization: ${String(value)}`);
}
...
function serializeBubbleAttributes(
  ...
  return Object.freeze(
    Object.fromEntries(
      Object.entries(attributes).sort(([leftKey], [rightKey]) => compareKeys(leftKey, rightKey)),
    ),
  );
}
    ...
    properties: Object.freeze(
      Object.fromEntries(
        Object.entries(node.properties)
          .sort(([leftKey], [rightKey]) => compareKeys(leftKey, rightKey))
          .map(([key, value]) => [key, serializeBubblePropertyValue(value)] as const),
      ),
    ),
    children: Object.freeze(node.children.map((childId) => projectSerializedNode(childId, nodes))),
  });
...
export function serializeBubbleSnapshot(snapshot: BubbleSnapshot): string {
  return JSON.stringify(projectSerializedNode(snapshot.rootId, snapshot.nodes), null, 2);
}
