  ...
  const runDueTimers = () => {
    while (true) {
      const dueTimers = Array.from(scheduledTimers.entries())
        ...
        .sort(
          ([, left], [, right]) =>
            left.dueTime - right.dueTime || left.sequence - right.sequence
        )
    ...
    network: {
      async fetch(request) {
        requests.push({
          ...request,
          headers:
            request.headers === undefined ? undefined : { ...request.headers },
        })
        ...
        if (nextScript === undefined) {
          throw new Error(
            `Unexpected network request: ${JSON.stringify(request)}`
          )
        }
...
function getSnapshotNodeOrThrow(
...
describe('createBubble', () => {
  ...
  test('creates isolated bubble instances', () => {
    ...
    const firstElementId = firstBubble.transact(tx =>
      tx.createElement({ tag: 'button' })
    )
    const secondElementId = secondBubble.transact(tx =>
      tx.createElement({ tag: 'button' })
    )
  ...
  test('viewport subscriptions are a no-op when the capability does not support them', () => {
    ...
    const unsubscribe = bubble.subscribeToViewport(() => {
      throw new Error(
        'Expected no viewport events without a subscription-capable viewport'
      )
    })
  ...
  test('keeps storage isolated between bubble sessions', () => {
    ...
    expect(firstBubble.resolveCapability('storage').getItem('token')).toBe(
      'first'
    )
    expect(
      secondBubble.resolveCapability('storage').getItem('token')
    ).toBeNull()
  })
  ...
  test('leaves the tree unchanged when a transaction throws during mutation application', () => {
    ...
    expect(() => {
      bubble.transact(tx => {
        ...
        tx.insertChild({
          parentId: initialTree.sectionId,
          childId: failedChildId,
          index: 0,
        })
  ...
  test('clicking a label forwards activation to an explicitly associated control', () => {
    ...
    const nodeIds = bubble.transact(tx => {
      ...
      tx.addEventListener({
        ...
        listener: event => {
          calls.push(
            `label:${event.targetId}:${event.currentTargetId}:${event.phase}`
          )
        },
      })
      tx.addEventListener({
        ...
        listener: event => {
          calls.push(
            `input:${event.targetId}:${event.currentTargetId}:${event.phase}`
          )
        },
      })
    ...
    expect(
      bubble.dispatchEvent({ type: 'click', targetId: nodeIds.labelId })
    ).toEqual({
      defaultPrevented: false,
      delivered: true,
    })
  ...
  test('serializes checkbox checked state in snapshots', () => {
    ...
    const inputIds = bubble.transact(tx => {
      ...
      tx.setAttribute({
        nodeId: checkedInputId,
        name: 'type',
        value: 'checkbox',
      })
      tx.setAttribute({
        nodeId: uncheckedInputId,
        name: 'type',
        value: 'checkbox',
      })
  ...
  test('includes only checked checkbox controls in form payloads', () => {
    ...
    const formId = bubble.transact(tx => {
      ...
      tx.insertChild({
        parentId: createdFormId,
        childId: defaultValueCheckboxId,
      })
  ...
  test('does not reuse IDs after node removal in the same session', () => {
    ...
    const { removedElementId, removedTextId, replacementId } = bubble.transact(
      tx => {
        const createdElementId = tx.createElement({ tag: 'button' })
        const createdTextId = tx.createText({ value: 'Save' })
  ...
  test('query locates existing and missing nodes by ID', () => {
    ...
    expect(snapshot.query.getById(buttonId)).toEqual(
      getSnapshotNodeOrThrow(snapshot, buttonId)
    )
  ...
  test('derives the textbox role for textarea elements', () => {
    ...
    const textareaId = bubble.transact(tx =>
      tx.createElement({ tag: 'textarea' })
    )
  ...
  test('falls back to a null role for unknown elements', () => {
    ...
    const sectionId = bubble.transact(tx =>
      tx.createElement({ tag: 'section' })
    )
  ...
  test('applies supported tabIndex overrides before natural DOM order', () => {
    ...
    const {
      ...
      tx.setAttribute({
        nodeId: createdAttributeButtonId,
        name: 'tabindex',
        value: '3',
      })
      tx.setProperty({
        nodeId: createdPropertyInputId,
        name: 'tabIndex',
        value: 1,
      })
      ...
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdDefaultButtonId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdAttributeButtonId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdPropertyInputId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
  ...
  test('skips disabled elements when computing tab order', () => {
    ...
    const { enabledButtonId, enabledInputId } = bubble.transact(tx => {
      ...
      tx.setAttribute({
        nodeId: createdDisabledButtonId,
        name: 'disabled',
        value: '',
      })
      tx.setProperty({
        nodeId: createdDisabledInputId,
        name: 'disabled',
        value: true,
      })
      ...
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdEnabledButtonId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdDisabledButtonId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdDisabledInputId,
      })
      tx.insertChild({
        parentId: bubble.rootId,
        childId: createdEnabledInputId,
      })
  ...
  test('rejects invalid child mutations', () => {
    ...
    const otherElementId = otherBubble.transact(tx =>
      tx.createElement({ tag: 'dialog' })
    )
  ...
  test('rejects attribute mutations on unsupported node types', () => {
    ...
    bubble.transact(tx => {
      ...
      expect(() => {
        tx.setAttribute({
          nodeId: bubble.rootId,
          name: 'role',
          value: 'application',
        })
      }).toThrow(
        `Attributes are only supported on element nodes: ${bubble.rootId}`
      )
      expect(() => {
  ...
  test('rejects property mutations on unsupported node types', () => {
    ...
    bubble.transact(tx => {
      ...
      expect(() => {
        ...
        `Properties are only supported on element nodes: ${bubble.rootId}`
      )
      expect(() => {
      ...
      expect(() => {
        ...
        `The value property is only supported on text input elements: ${buttonId}`
      )
      expect(() => {
        tx.setProperty({ nodeId: buttonId, name: 'checked', value: true })
      }).toThrow(
        `The checked property is only supported on checkbox input elements: ${buttonId}`
      )
