...
function createHarness(options?: Parameters<typeof mountTodoApp>[0]) {
  ...
  const attachedTree = (): BubbleSerializedNode =>
    JSON.parse(serializeBubbleSnapshot(app.bubble.snapshot())) as BubbleSerializedNode;
  ...
  const paragraphId = (): string => app.bubble.snapshot().query.getByTag("p")[0]!.id;
...
describe("mountTodoApp", () => {
  test("renders whatever the store currently holds on first mount", () => {
    const { attachedLis, attachedTree, assertions, paragraphId } = createHarness({
      initialTodos: [
  ...
  test("'Add sample todo' clicks cycle through the sample labels via the store", () => {
    const { attachedLis, attachedTree, click } = createHarness({ initialTodos: [] });
